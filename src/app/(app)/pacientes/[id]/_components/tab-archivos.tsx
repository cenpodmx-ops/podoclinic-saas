...
export function TabArchivos({ patient }: { patient: Patient }) {
  ...
  const files: PatientFileRow[] = data?.data || []
        ...
        xhr.onload = () => {
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            try {
              ...
              reject(new Error(e.error || 'Error al subir archivo'))
            } catch {
  ...
  async function deleteFile(file: PatientFileRow) {
    ...
    try {
      const res = await fetch(`/api/pacientes/${patient.id}/archivos/${file.id}`, { method: 'DELETE' })
      if (!res.ok) {
        ...
        <CardContent className="p-4 space-y-3">
          ...
          >
            ...
            <p className="text-xs text-muted-foreground mt-1">
              PDF, JPG, PNG o DOCX · Máximo 20MB
            </p>
            <input
              ref={inputRef}
              type="file"
